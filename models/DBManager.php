...
$db = new DBManage ( 'localhost', 'root', 'root', 'test', 'utf-8' );
...
class DBManager
{
    ...
    function backup($tablename = '', $dir = '', $size = 2000)
    {
        ...
        if (!empty ($tablename ))
        {
            ...
            while ( $record = mysql_fetch_array ( $data ) )
            {
                ...
                if (strlen ( $sql ) >= $size * 1000)
                {
                    ...
                    {
                        $return .=  "表-" . $tablename . "-卷-" . $p . "-数据备份完成,生成备份文件 <span style='color:#f00;'>$dir$filename</span><br />";
                    }
                    else
                    {
                        $return .=  "备份表-" . $tablename . "-失败<br />";
                    }
                    // 下一个分卷
                    $p ++;
        ...
        { // 备份全部表
            ...
            while ( $table = mysql_fetch_array ( $tables ) )
            {
                ...
                while ( $record = mysql_fetch_array ( $data ) )
                {
                    ...
                    if (strlen ( $sql ) >= $size * 1000)
                    {
                        ...
                        if ($this->_write_file ( $sql, $file, $dir ))
                        {
                            $return .=   "-卷-" . $p . "-数据备份完成,生成备份文件<span style='color:#f00;'>$dir$file</span><br />";
                        }
                        else
                        {
                            $return .=   "备份卷-" . $p . "-失败<br />";
            ...
            if ($sql != "")
            {
                ...
                if ($this->_write_file ( $sql, $filename, $dir ))
                {
                    $return .=   "-卷-" . $p . "-数据备份完成,生成备份文件 <a href='http://" . $_SERVER['SERVER_NAME'] . "/db_backup/$filename'><span style='color:#f00;'>$filename</a><br />";
                }
                else
                {
                    $return .=   "备份卷-" . $p . "-失败<br />";
    ...
    private function _write_file($sql, $filename, $dir) {
        $dir = $dir ? $dir : dirname(__FILE__) . '../web/db_backup/';
        // 不存在文件夹则创建
        if (! file_exists ( $dir )) {
            mkdir ( $dir );
        }
...
//$db = new DBManage ( 'localhost', 'root', 'root', 'www100', 'utf-8' );
//$db->backup ('');   //全部备份用这个
//$db->restore ( './backup/20151111174526_all_v1.sql');//还原备份用这个
?>